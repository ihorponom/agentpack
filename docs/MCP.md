...
`task_finalize` closes the current Task Passport only after verification is already `passed`, `failed`, or `accepted`, or when that final status is passed explicitly with `status`. It also accepts `evidence` IDs and a short `summary`, matching the CLI `task finalize` command. It reports the bound HEAD and completed/frozen consequence, preserves an already-final verdict's bound HEAD, and binds the live HEAD for direct finalization from non-final verification. Accepted finalization refuses tasks with remaining next actions unless `force: true` is passed; use `task_park` for deferred work. The result appends the same non-blocking hygiene and risk-proportional verification advisories as the CLI (uncommitted in-scope changes, remaining next actions, missing checkpoint, or unusable evidence; see docs/TASK-PASSPORT.md). For low risk, evidence can be a short readable note or useful test output covering the check, result, and relevant limits. Code scopes include a `Reviewed HEAD` exactly matching the Passport-bound SHA. Medium/high retains independent read-only review and a named disconfirming check. After risk calibration, both surfaces report exact missing or malformed labels for the best incomplete candidate and a copy-ready template. These are structural advisories, not semantic correctness judgments or lifecycle gates.
