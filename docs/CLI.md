...
`doctor` checks pack setup, local integration config (including native Codex and Cursor task gates), git availability, and source-cache health. A gate launcher generated by another stable Agentpack version in the supported `>=1.2.0 <2.0.0` range is structurally compatible: `doctor` parses the exact generated command shape and checks accessible non-empty Node/Agentpack files, package identity, the declared CLI entrypoint, and the version without executing the configured command. This is a conservative configuration check, not a runtime execution probe. Missing, moved, malformed, or incompatible launchers still warn with the matching reinstall command. Changed or missing source records are warnings, not setup failures; use `agentpack source status --changed --missing` to review details before a release-like handoff.
...
`task audit [--json]` checks the current passport for branch/head drift, missing next actions, open verification, closed-task anomalies, source-cache metadata drift, and advisory-only risk-proportional adversarial evidence. Low-risk success expects a concrete self-challenge; medium/high-risk success expects independent read-only review, a named negative/differential/operational/rollback check, and a `Reviewed HEAD` exactly matching the Passport-bound SHA for code scopes. Contract-changing work should be classified medium/high. Generic confirmatory prose is insufficient. After risk calibration, incomplete referenced evidence produces exact missing or malformed labels for the best candidate and a copy-ready template; the advisory neither judges technical correctness nor blocks. Metadata warnings are shown separately so they do not look like action-required task failures. Its additive `ceremonyDiagnostics` field only reports bounded review candidates, never lifecycle violations or claimed savings.
